...
public class ParserConfigImpl implements ParserConfig {
    ...
    private final Set<Character> placeholderCharacters;
    ...
    private final boolean supportLegacyFormat;
    ...
    public ParserConfigImpl(
        ...
        this.supportLegacyFormat = supportLegacyFormat;
        this.placeholderPatterns = this.initPatterns();
        this.placeholderCharacters = new HashSet<>(this.placeholderFormats.size() * 2);
    ...
    private List<Pattern> initPatterns() {
        ...
        for (final Pair<Character, Character> placeholderFormat : this.placeholderFormats) {
            result.add(Pattern.compile(
                    Pattern.quote(placeholderFormat.getLeft() + "") + "([a-zA-Z_0-9]+)"
                            + Pattern.quote(placeholderFormat.getRight() + ""),

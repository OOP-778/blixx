...
public class ObjectArray<T> implements Iterable<T> {
    ...
    public ObjectArray(Iterable<T> iterable) {
        this(
                iterable instanceof ObjectArray<?>
                        ? ((ObjectArray<T>) iterable)
                        : (ObjectArray<T>) new ObjectArray<>(((Collection<T>) iterable).toArray(new Object[0])));
    }

    @SuppressWarnings("unchecked")
    public static <T> ObjectArray<T> empty() {
        return (ObjectArray<T>) EMPTY;
    }
    ...
    public void replaceOrAdd(Predicate<T> predicate, T value) {
        final int size = this.size;
        for (int i = 0; i < size; i++) {
            final T object = this.array[i];
            if (object == null) {
    ...
    public ObjectArray<T> copy(Function<T, T> objectCloner) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            copy[i] = objectCloner.apply(object);
        }
